...
def animate(_):
    ...
    if (len(initial_errors) != len(final_errors)) and (len(initial_errors) != len(evaluation_errors)) and (
            len(initial_errors) != len(final_scores)):
        return
    ...
    initial_errors_moving_average_plot, = top_right_axes.plot(initial_errors_moving_average, 'y-',
                                                              label="initial errors moving average")
    final_errors_moving_average_plot, = top_right_axes.plot(final_errors_moving_average, 'y-',
                                                            label="final errors moving average")
    evaluation_errors_moving_average_plot, = top_right_axes.plot(evaluation_errors_moving_average, 'y-',
                                                                 label="evaluation errors moving average")
    top_right_axes.fill_between(range(len(initial_errors_moving_average)), initial_errors_moving_average,
                                final_errors_moving_average, alpha=0.7)
    ...
    final_scores_moving_average_plot, = bottom_right_axes.plot(final_scores_moving_average, 'y-',
